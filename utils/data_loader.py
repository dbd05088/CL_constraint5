...
class MemoryDataset(Dataset):
    ...
    def classwise_get_weight(self):
        weight = np.zeros(len(self.images))
        weight = weight.astype('float64')
        
        for i in range(len(weight)):
            weight[i] = self.cls_weight[self.labels[i]]
        
        # option 1 softmax 취해주기 / option 2 전체 
        if self.weight_option == "softmax":
            ...
            cls_loss = copy.deepcopy(self.cls_loss)
            for i in range(len(cls_loss)):
                if cls_loss[i] is None:
                    cls_loss[i] = 1
            
            for i in range(len(weight)):
                weight[i] = cls_loss[self.labels[i]]
            weight_tensor = torch.DoubleTensor(weight)
            weight = F.softmax(weight_tensor, dim=0)
            
        return weight

    def update_loss(self, c, cls_loss):
        if self.cls_loss[c] is not None:
            self.cls_loss[c] = self.weight_ema_ratio * cls_loss  + (1-self.weight_ema_ratio) * self.cls_loss[c]
        else:
            self.cls_loss[c] = c

    def update_cls_loss(self, cls_list, cls_loss):
        loss_dic = {}
        for i, c in enumerate(cls_list):
            if c not in loss_dic.keys():
                loss_dic[c] = cls_loss[i]
            else:
                loss_dic[c].append(cls_loss[i])
        for c in list(loss_dic.keys()):
            c_loss = torch.mean(loss_dic[c]).item()
            self.update_loss(c, c_loss)
                
    
    def decrease_weight(self, cls_idx):
        self.cls_weight[cls_idx] *= self.cls_weight_decay

    def samplewise_get_weight(self):
        weight = np.zeros(len(self.images))
        weight = weight.astype('float64')
    ...
    @torch.no_grad()
    def get_batch(self, batch_size, stream_batch_size=0, use_weight=False, transform=None, twf=False, recent_ratio=None, exp_weight=False, use_human_training=False):
        ...
        if memory_batch_size > 0:
            if use_weight:
                weight = self.samplewise_get_weight()
                indices = np.random.choice(range(len(self.images)), size=memory_batch_size, replace=False, p= weight)
                # weight decay for selected indices
                
                '''
                for i in indices:
                    self.decrease_weight(self.labels[i])
                '''
            else:
        ...
        if twf:
            buf_data['logits'] = logits
            buf_data['d'] = d
            buf_data['task_ids'] = torch.Tensor(task_ids)
            return data, buf_data
        else:
            return data
